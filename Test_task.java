...
public class DocumentManager {
    ...
    public Document save(Document document) {
        ...
        // I`m was not sure about getAuthor() check , but I decided to check it for null.
    ...
    public List<Document> search(SearchRequest request) {
        if (request == null){
            ...
            return storage.values()
                    .stream()
                    .filter(document -> matchesRequest(document,request)).toList();
    }
    private boolean matchesRequest(Document document , SearchRequest request){
        if (request.getTitlePrefixes() != null && !request.getTitlePrefixes().isEmpty()) {
            boolean matchesPrefix = request.getTitlePrefixes().stream()
